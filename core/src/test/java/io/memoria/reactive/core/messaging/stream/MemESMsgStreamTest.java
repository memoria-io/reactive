...
class MemESMsgStreamTest {
  ...
  void publishAndSubscribe() {
    ...
    // And
    StepVerifier.create(stream.last(topic, 0)).expectNext(s0LastMessage.key()).verifyComplete();
    StepVerifier.create(stream.last(topic, 1)).expectNext(s1LastMessage.key()).verifyComplete();
  }
